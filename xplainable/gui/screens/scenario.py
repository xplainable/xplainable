...
import numpy as np
from ..components import Metric, KeyValueTable
...
class Scenario:
    ...
    def __init__(self, model, ranges={}):
        ...
        self.score = None
        self.proba_card = None
        self.score_card = None
        self.multi_card = None
    ...
    def generate_visuals(self):
        ...
        values = {
            "Probability": f'{round(p, 2)}%',
            "Score": str(s),
            "Multiplier": f'{m}x',
            "Support": str(support)
        }

        self.kvtable.update_data(values)
        ...
        df = pd.DataFrame.from_dict(metadata, orient='index', columns=['contribution'])
        ...
        if self.toggle_sort.value:
            ...
            df = pd.concat([pd.DataFrame(df.iloc[0].copy()).T, slc, pd.DataFrame(df.iloc[length-1].copy()).T])
        ...
        if len(_remainder) > 0:
            ...
            data = pd.concat([df.iloc[_filter].copy(), remainder_df, pd.DataFrame(df.iloc[length-1].copy()).T], ignore_index=True).reset_index(drop=True)
        ...
        data['colour'] = data['contribution'].apply(lambda x: '#12b980' if x > 0 else '#e14067')
        ...
        bars = sns.barplot(data=data, y='feature', x='upper', orient='horizintal', palette=data['colour'], ax=ax[0])
        bars2 = sns.barplot(data=data, y='feature', x='lower', orient='horizintal', color='white', ax=ax[0])
        ...
        support_data = pd.DataFrame(self.model._support_map.items(), columns=['score', 'support'])
        ...
        cp_line = sns.lineplot(data=plot_data, y='probability', x='score', ax=ax[1])
        ...
        cp2 = ax[1].twinx()
        axx = sns.lineplot(data=support_data, x='score', y='support', ax=cp2, color='#0080ea', alpha=0.1)
    ...
    def get_feature_values(self):
        vals = {c: self.widgets[c].value if c in self.widgets else 0 for c in self.model.columns}
        return vals
    ...
    def run(self):
        ...
        for f in list(profile['categorical'].keys())+list(profile['numeric'].keys()):
            if f not in self.widgets:
                continue
            title = widgets.HTML(f'<h5>{f}</h5>')
            w = self.widgets[f]
            box = widgets.VBox([title, w])
            box.layout = widgets.Layout(overflow='hidden', flex='none', max_width='330px')
            selector_children.append(box)
        
        selectors = widgets.VBox(selector_children)
        selectors.layout = widgets.Layout(height='650px', flex='none', overflow='hidden')
        ...
        self.kvtable.html_widget.layout = widgets.Layout(margin='0 0 0 155px')
        ...
        self.feature_filter.value = (0, min([10, len(self.widgets)]))
        ...
        output = widgets.VBox(
            [self.kvtable.html_widget, divider, waterfall_zone])
