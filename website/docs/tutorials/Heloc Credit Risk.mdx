...
```python
# Note: This dataset requires minimal preprocessing - all features are numeric
# and there are no ID columns to drop. The PipelineSpec is set up for completeness
# and to provide a framework for adding future preprocessing steps.

preprocessing_spec = PipelineSpec(
    target_column="RiskPerformance",
    steps=[
        # No additional preprocessing steps needed for this dataset.
        # All features are numeric credit risk indicators.
        # Add StepSpec entries here if future preprocessing is required, e.g.:
        # StepSpec(action="drop_columns", columns=["unwanted_column"]),
    ]
)

# Compile and apply the preprocessing spec
...
```python
X, y = df_transformed.drop(columns=['RiskPerformance']), df_transformed['RiskPerformance']
...
<iframe src='/plot_data/AltairPlot_8f0a6770-8cd0-4c00-a2c8-926a5667f8fb.html' width='100%' height='520' style={{border: '1px solid #ccc', borderRadius: '4px'}}></iframe>
...
try:
    preprocessor_id, preprocessor_version_id = client.preprocessing.create_preprocessor(
        name="HELOC Credit Risk Preprocessing",
        description="Preprocessing specification for the HELOC Credit Risk dataset. Minimal preprocessing as all features are numeric credit risk indicators.",
