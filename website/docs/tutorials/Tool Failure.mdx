...
preprocessing_spec = PipelineSpec(steps=[
    StepSpec(
        ...
        params={"columns": [
            ...
            "UDI",          # Unique identifier - not predictive
            "TWF",          # Failure sub-type - data leakage
            "HDF",          # Failure sub-type - data leakage
            "PWF",          # Failure sub-type - data leakage
            "OSF",          # Failure sub-type - data leakage
            "RNF",          # Failure sub-type - data leakage
        ]},
        description="Drop identifier columns and failure sub-type columns (data leakage)"
    ),
])
...
try:
    preprocessor_id, preprocessor_version_id = client.preprocessing.create_preprocessor(
        name="Tool Failure Preprocessing",
        description="Drop identifier and failure sub-type columns from the asset failure dataset to prevent data leakage",
...
<iframe src='/plot_data/AltairPlot_b36ff4a0-d71b-4e63-ada6-625386f9bfbf.html' width='100%' height='520' style={{border: '1px solid #ccc', borderRadius: '4px'}}></iframe>
...
<iframe src='/plot_data/AltairPlot_3a764d80-f1c7-4e26-911d-f55f6660d7a1.html' width='100%' height='520' style={{border: '1px solid #ccc', borderRadius: '4px'}}></iframe>
...
The code block illustrates the deployment of our prediction model using the
...
1. **Activating the Deployment:** The model deployment is activated using
   `client.deployments.activate_deployment`, which changes the deployment status to
   active, allowing it to accept prediction requests.
...
3. **Generating Example Payload:** An example payload for a deployment request is
   generated by `client.deployments.generate_example_deployment_payload`. This payload
   mimics the input data structure the model expects when making predictions.
