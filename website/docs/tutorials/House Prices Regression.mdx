...
if 'Id' in train_df.columns:
    steps.append(StepSpec(
        transformer="DropColumnsTransformer",
        params={"columns": ["Id"]}
    ))
...
# Build and compile the pipeline spec
spec = PipelineSpec(steps=steps)
pipeline = compile_spec(spec)

# Apply preprocessing
df_processed = pipeline.fit_transform(train_df)
...
#         spec=spec,
#         name="House Prices Preprocessor",
#         description="Drops Id column, fills missing numeric values with median"
#     )
