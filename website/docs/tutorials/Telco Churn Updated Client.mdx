...
# Create preprocessing pipeline spec
...
print(f"Pipeline spec created with {len(preprocessing_spec.steps)} steps")

# Compile and apply preprocessing
print("Compiling and applying preprocessing pipeline...")
...
try:
    ...
    # Create preprocessor with type-safe response using PipelineSpec
    preprocessor_id, preprocessor_version_id = client.preprocessing.create_preprocessor(
        name=f"Telco Churn Preprocessing - v{xp.__version__} - New Client",
        description="Complete preprocessing pipeline for IBM Telco Churn Dataset with refactored client",
    ...
    print(f"Version ID: {preprocessor_version_id}")
    ...
    preprocessor_id, preprocessor_version_id = "local-pipeline", "local-version"
